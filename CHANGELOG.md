...
RHEL-09-211045 (addresses #38; thank you @mbc3): restored the `.conf`-only drop-in glob on this branch and rewrote the exec so grep's exit code no longer decides the verdict. The v2r8 variant of the #30 fix greps /etc/systemd/system.conf.d/* (all filenames, correct there because the v2r8 remediation drop-in carries no extension) and reached this branch through the latest -> benchmark_v2r9 merge, silently reverting the V2R9 `.conf` strictness. The check now greps /etc/systemd/system.conf plus system.conf.d/*.conf with `grep -hs ... || true`: -h stops grep prefixing filenames (which would break the ^-anchored stdout matchers), -s replaces the 2>/dev/null redirect so the useless cat could go, and `|| true` pins exit-status to 0 so a missing setting yields one stdout failure instead of an exit-status failure plus a matcher failure. Both matchers now tolerate optional whitespace around the key and the =, and the negative matcher was confirmed complete against systemd-system.conf(5) (the only non-none values are reboot-force, poweroff-force, reboot-immediate and poweroff-immediate). A suffixless drop-in is no longer accepted: systemd reads only *.conf files under system.conf.d/ (conf_files_list_dropins passes a ".conf" suffix), so such a file never takes effect, and the V2R9 remediation role writes 55-CtrlAltDel-BurstAction.conf.
...
RHEL-09-611195/611200 (addresses #30): rewrote the emergency/rescue authentication checks from a file resource that hardcoded one drop-in filename (emergency.service.conf/rescue.service.conf, exists:true) to a command resource that greps both the vendor unit (/usr/lib/systemd/system/{emergency,rescue}.service) and every drop-in (.../*.conf), matching the DISA check (the setting is compliant in either the default unit or any drop-in file). Fixes a false finding on hosts that carry the setting via the systemd-standard override.conf or the unmodified vendor unit
RHEL-09-611170/211045/231195 (same drop-in-path bug class): same file -> command conversion for the sssd certificate_verification, CtrlAltDelBurstAction, and cramfs-blacklist checks, which likewise hardcoded a single .conf path where the DISA check greps multiple locations (sssd.conf + conf.d/, system.conf + system.conf.d/, modprobe.conf + modprobe.d/); each now passes regardless of which file provides the setting, and the 211045/231195 positive+negative content matchers are preserved as command stdout matchers
